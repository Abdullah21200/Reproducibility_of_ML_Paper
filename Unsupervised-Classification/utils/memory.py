...
class MemoryBank(object):
    def __init__(self, n, dim, num_classes, temperature, feature_dim=512):
        ...
        self.penultimate_layer_output = torch.FloatTensor(self.n, feature_dim)
    ...
    def update(self, features, pen_layer, targets):
        b = features.size(0)
        
        assert(b + self.ptr <= self.n)
        
        self.features[self.ptr:self.ptr+b].copy_(features.detach())
        self.penultimate_layer_output[self.ptr:self.ptr+b].copy_(pen_layer.detach())
        self.targets[self.ptr:self.ptr+b].copy_(targets.detach())
        self.ptr += b

    def to(self, device):
        self.features = self.features.to(device)
        self.penultimate_layer_output = self.penultimate_layer_output.to(device)
